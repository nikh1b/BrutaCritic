...
type Section = "home" | "profile" | "leaderboard" | "search" | "admin" | "audio" | "reviews" | "privacy";
...
function App() {
  ...
  return (
    <ErrorBoundary>
      <div className="min-h-screen bg-background text-text-primary selection:bg-critic-green selection:text-black flex flex-col">
        <Navbar
        ...
        <main className="pt-24 px-4 max-w-7xl mx-auto mb-20 flex-1 w-full">
          {/* Quick Nav */}
          {activeSection !== "home" && (
          ...
          {activeSection === "privacy" && (
            <section className="py-12">
              <PrivacyPolicy onBack={() => setActiveSection("home")} />
            </section>
          )}
        ...
        {/* Footer */}
        <footer className="w-full py-8 border-t border-zinc-800 bg-black text-center text-sm text-zinc-600">
          <div className="max-w-7xl mx-auto px-4 flex flex-col md:flex-row justify-between items-center gap-4">
            <p>© {new Date().getFullYear()} BrutaCritic. No Paid Shills.</p>
            <div className="flex items-center gap-6">
              <button onClick={() => setActiveSection("privacy")} className="hover:text-white transition-colors">Privacy Policy</button>
              <button className="hover:text-white transition-colors">Terms of Service</button>
              <a href="https://github.com/nikh1b/BrutaCritic" target="_blank" rel="noopener noreferrer" className="hover:text-white transition-colors">GitHub</a>
            </div>
          </div>
        </footer>
      </div>
